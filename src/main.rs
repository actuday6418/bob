...
fn string_space_remover(query: String) ->String{
    let mut i: usize = 0;
    let mut j: usize;
    let mut flag: bool;
...
fn comment_remover(query: &str) -> String{
    let mut i: usize = 0;
    let mut j: usize;
    let mut flag: bool;
...
fn bob_and_punctuation_remover(query: String) -> String{
...
fn main() {
    let mut query = String::new();//"Bob write(trying to work something out) \"hello i am bob\"";
    io::stdin().read_line(&mut query);
    let mut query = &(query.trim());
    let mut query: String = comment_remover(query);
    let mut query: String = string_space_remover(query);
    let mut query: String = bob_and_punctuation_remover(query);
    println!("{}",query);
    let mut query_vector: Vec<String> = query.split_whitespace().map(String::from).collect();
    //lib::line_iterate(&mut query_vector);
}
